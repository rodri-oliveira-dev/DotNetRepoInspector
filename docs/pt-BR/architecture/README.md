...
- **MCP** — adapter local, read-only e planejado para Model Context Protocol sobre `IRepositoryInspector` para clientes de agentes/IDEs. É um adapter de delivery, não um segundo engine de inspeção.
...
O servidor MCP planejado segue a [ADR 0006](../decisions/0006-mcp-adapter-architecture.md): usa stdio, uma fronteira explícita de repository root, tools read-only derivadas de `InspectionReport`, uma identidade de pacote `DotNetRepoInspector.Mcp` separada e compatibilidade no nível do protocolo em vez de SDKs específicos de providers de LLM.
