...
- The only tool in this increment is `inspect_repository`. It maps its options to the existing `RepositoryInspectionRequest` and returns the canonical `InspectionReport` without interpretation or summarization.
...
The executable host owns startup parsing, root validation, DI, MCP metadata, stdio transport, stderr logging, and process lifetime. `InspectRepositoryHandler` owns adapter validation and maps the tool call to `RepositoryInspectionRequest`. `RepositoryInspector` remains the only inspection orchestrator and `InspectionJsonSerializer` remains the canonical report serialization boundary.
...
The host uses `ModelContextProtocol` 2.2.0 and `Microsoft.Extensions.Hosting` 10.0.12 through Central Package Management. The MCP project references Core and Engine, while Core and Engine have no MCP dependency. Packaging and publication remain deferred to the distribution roadmap items.
