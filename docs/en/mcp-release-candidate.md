...
The RC must use the protected release workflow and the same exact version for the CLI, MCP package, GitHub Action, container, manifest, and release tag. Entry requires completed #133, #134, #135, #136, and #139; green MCP-specific repository gates; and no untreated critical MCP security issue. Container release-readiness tracked by #106 is independent and does not gate the MCP release candidate. Promotion to GA is prohibited until the published package is resolved by exact version from NuGet.org and at least two real MCP clients from different providers pass against that package.
